...
function proc_feedburner_email_subscription( $args ) { 
	...
	<form action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $feed; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
		<p>
			...
			<input class="btn" type="submit" name="imageField" value="<?php echo $submit; ?>" alt="Submit" />
	...
	<?php
		...
		if( $args['posts_feed_link'] )
			echo '<a class="feed-link" title="' . esc_attr(sprintf( $rss['feedtitle'], get_bloginfo('name'), $rss['separator'] )) . '" href="' . get_feed_link() . '"><img alt="i" src="' . FEEDBURNER_EMAIL_SUBSCRIPTION_URL . 'images/rss.png" />Posts <abbr title="Really Simple Syndication">RSS</abbr></a>';
		
		if( $args['comments_feed_link'] )
			echo '<a class="feed-link" title="' . esc_attr(sprintf( $rss['comstitle'], get_bloginfo('name'), $rss['separator'] )) . '" href="' . get_feed_link( 'comments_' . get_default_feed() ) . '"><img alt="i" src="' . FEEDBURNER_EMAIL_SUBSCRIPTION_URL . 'images/rss.png" />Comments <abbr title="Really Simple Syndication">RSS</abbr></a>';
	?>
